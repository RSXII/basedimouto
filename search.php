...
//sanitize the phrase
$phrase = clean_string( $_GET['the_search']);

//parse the search form if the phrase is not blank
if( $phrase != '' ){
    $query = "SELECT title, date_created, body, post_id FROM posts WHERE ( title LIKE '%$phrase%' OR body LIKE '%$phrase%' ) AND is_published = 1 ORDER BY date_created DESC";
            ...
            <section class="results">
                ...
                <?php while( $row = $result->fetch_assoc() ){ ?>
                    <article class="searchresults">
                        <h2><a href="post.php?post_id=<?php echo $row['post_id']; ?>">
                                <?php echo $row['title']; ?>
                            </a></h2>
